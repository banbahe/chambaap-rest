...
namespace chambapp.bll.Users
{
    public class UserBll : IUserBll
    {
        ...
        public async Task<ResponseModel> SetAsync(RecruiterDto recruiterDto = null, CandidateDto candidateDto = null)
        {

            try
            {
                // MAPPER 
                var mapperUser = (recruiterDto == null) ? _mainMapper.Mapper.Map<User>(candidateDto ) :
                                                          _mainMapper.Mapper.Map<User>( recruiterDto); 

                // DAL
                var result = await _userDal.SetAsync(mapperUser);
        ...
        public async Task<ResponseModel> CreateRecruiterAsync(RecruiterDto recruiterDto)
        {
            ...
            try
            {
                ...
                // DAL
                var result = await _userDal.CreateAsync(mapperUser);
                _responseModel.Datums = result;
            }
            catch (Exception ex)
            {
